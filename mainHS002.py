...
# Thiết bị số 2 là đèn chống trộm cho HS002
IDDENCHONGTROM = 2

# Cập nhật ID thiết bị theo cấu hình HS002
current_device_status = {
    IDDENCHONGTROM: {"status": False, "type": "denchongtrom"},
    1: {"status": False, "type": "den"},
    7: {"status": 0, "type": "servo"},
    6: {"status": 0, "type": "quat"}
}
...
while True:
    ...
    if time.ticks_diff(time.ticks_ms(), last_api_call) > 500:
        try: 
            response = urequests.get(COMMANDS_URL + "?houseid=" + HOUSEID)
